...
class Sudoku {
  ...
  solve() {
    ...
    while (i < emptyCoordinate.length) {
      ...
      let guessNum = arrBoard[row][col] + 1; // 1-9
      //Try num 1 to 9 to until the solution is found
      while (!proceedStatus && guessNum <= 9) {
        if (this.checkAll(arrBoard, row, col, guessNum)) {
          proceedStatus = true;
      ...
      //For backtracking if the solution for current iteration isn't found
      if (!proceedStatus) {
        arrBoard[row][col] = 0;
        i--;
      }
  ...
  checkAll (arrBoard, row, col, guessNum) {
    if (this.checkRow(arrBoard, row, guessNum) && this.checkCol(arrBoard, col, guessNum) && this.checkGroup(arrBoard, row, col, guessNum)) {
      return true;
    }
    return false;
  }
